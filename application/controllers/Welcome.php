...
class Welcome extends CI_Controller {
	...
	{ 
		...
		$printer = new Printer($connector);
		   function buatBaris4Kolom($kolom1, $kolom2, $kolom3, $kolom4, $kolom5) {
            // Mengatur lebar setiap kolom (dalam satuan karakter)
            $lebar_kolom_1 = 12;
            $lebar_kolom_2 = 8;
            $lebar_kolom_3 = 9;
		...
		try { 
			  ...
			  // Data transaksi
		        ...
		        $printer->initialize(); // Reset bentuk/jenis teks
		        $printer->text("--------------------------------------------------\n");
		        $printer->text(buatBaris4Kolom("Barang", "qty", "Harga", "Dis", "Subtotal"));
		        $printer->text("--------------------------------------------------\n");
		        foreach ($data->data[0]->produk as $v) { 
		        $printer->text(buatBaris4Kolom($v->produk , $v->qty. ' '.$v->satuan, $v->harga, '-'.$v->diskon , $v->harga_total )); 
		    	}
		        $printer->text("--------------------------------------------------\n");
		        $printer->text(buatBaris4Kolom('', '', '', "Total", $data->data[0]->total->harga_asli)); 
		        $printer->text(buatBaris4Kolom('', '', '', "Diskon", $data->data[0]->total->diskon)); 
